...
## CRITICAL: Portability and Sanitization Rules

### When Referencing External Files for Genie Updates:
1. **ALWAYS sanitize project names** - Never mention specific project names from other folders
2. **Extract patterns, not specifics** - Focus on implementation patterns and structures
3. **Maintain internal paths** - All Genie paths must use `/home/neo/genie/` as base
4. **Generic descriptions only** - Describe external references as "source implementation" or "reference pattern"
5. **No cross-contamination** - Never copy project-specific configurations or names into Genie

### Example Approach:
- ❌ BAD: "Copy the save implementation from aae-bot"
- ✅ GOOD: "Update based on the reference implementation pattern"

### Internal File Routing:
- All Genie files use absolute paths from `/home/neo/genie/`
- Deployment targets use relative paths from target project root
- Templates remain project-agnostic with placeholders
...
```
target-project/
├── package.json              # NPM scripts (save, load) - updated/created
├── claude.json               # Claude quickCommands configuration
├── scripts/
...
### Usage in Deployed Projects:
- **NPM Scripts**: `npm run save` and `npm run load`
- **Claude Commands**: Type `save` or `load` directly in Claude Code
...
4. Run deployment: `node /home/neo/genie/bin/deploy.js`
5. Verify generated files exist and have proper content
6. Test commands: `npm run save` and `npm run load`
7. Verify claude.json quickCommands work
8. Check archive functionality after threshold reached
...
### When Updating Genie from External References:
1. **Review external patterns** without copying project names
2. **Extract generic logic** that applies to all projects
3. **Test in isolation** using `/tmp/test-project` directories
4. **Validate portability** by deploying to multiple project types
5. **Document changes** using generic terminology

### File Path Guidelines:
- Genie source: `/home/neo/genie/` (absolute)
- Test projects: `/tmp/genie-test-*` (temporary)
- Deployed files: `./scripts/`, `./docs/project/` (relative to target)
- Never hardcode paths to specific external projects